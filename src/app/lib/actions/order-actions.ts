    ...
    if (repeated) {
      ...
      const insertedHeader = await client.query(
        `INSERT INTO order_header (
           order_id, total_amount, shipping_amount, status, marketplace,
           document_type, has_invoice, delivery_date, invoice_uploaded,
           return_status, company_rut, billing_city, created_at
         )
         VALUES ($1, 0, $2, $3, $4, 'boleta', false, $5, false,
                 'sin_devolucion', NULL, NULL, COALESCE($6::timestamptz, NOW()))
         RETURNING id`,
        [
          order.orderId,
          order.shippingAmount,
...
export interface ShopifyOrderItemInput {
  marketplaceItemId: string;
  productTitle: string;
  productQuantity: number;
  productPrice: number;
...
export interface ShopifyOrderInput {
  ...
  status: StandardOrderStatus;
  deliveryDate?: string | null;
  items: readonly ShopifyOrderItemInput[];
}

export async function upsertShopifyOrder(order: ShopifyOrderInput) {
  const client = await pool.connect();
  ...
  try {
    ...
    const existingHeader = await client.query(
      `SELECT id, status, has_invoice, document_type
       ...
       WHERE order_id = $1 AND marketplace = $2
       FOR UPDATE`,
      [order.orderId, MARKETPLACES.SHOPIFY],
    );

    let orderHeaderId: string;
    const repeated = (existingHeader.rowCount ?? 0) > 0;

    if (repeated) {
      const header = existingHeader.rows[0];
      orderHeaderId = header.id;
      const resolvedHeader = resolveShopifyExistingHeaderStatus(
        header.status,
        order.status,
      );
      if (!resolvedHeader.accepted) {
        console.warn(
          `[OrderStatus] Transicion rechazada para orden Shopify ${order.orderId}: ${header.status} -> ${order.status}`,
        );
      }
      ...
      await client.query(
        `UPDATE order_header
         SET status = $1,
             shipping_amount = CASE WHEN has_invoice THEN shipping_amount ELSE $2 END,
             delivery_date = $3,
             document_type = CASE WHEN has_invoice THEN document_type ELSE 'boleta' END,
             marketplace = $4,
             updated_at = NOW()
    ...
    for (const item of order.items) {
      ...
      // Migra una sola vez las filas del flujo Shopify anterior, que agrupaba
      // por titulo y no guardaba el ID estable del LineItem.
      if ((existingDetail.rowCount ?? 0) === 0) {
        existingDetail = await client.query(
          `SELECT id, status, marketplace_status
           FROM order_detail
    ...
    await client.query('ROLLBACK');
    console.error('Error al sincronizar orden de Shopify:', error);
    return { error: 'Error al sincronizar orden de Shopify' };
  } finally {
    client.release();
  }
}
