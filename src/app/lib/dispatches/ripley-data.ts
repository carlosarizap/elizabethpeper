...
import { getRipleySvcConnectionStatus } from '../ripley/svc-client';
import type { RipleyDispatchOrder, RipleyDispatchResponse } from './definitions';
...
export async function fetchRipleyDispatchOrders(): Promise<RipleyDispatchResponse> {
  const [result, integration] = await Promise.all([
    pool.query<RipleyRow>(
    `SELECT
       oh.id,
       oh.order_id,
     ...
     WHERE oh.marketplace = $1
       ...
       AND EXISTS (
         ...
         WHERE od.id_order_header = oh.id
           AND UPPER(COALESCE(od.marketplace_status, '')) = 'SHIPPING'
       )
  ...
  const orders: RipleyDispatchOrder[] = result.rows.map((row) => ({
    id: row.id,
